...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  let sorte = starWarsArr.sort((a, b) => Number(b.height) - Number(a.height));
  return sorte;
};
...
const removeThree = (idx, arr) => {
  ...
  for(let i=idx+idx+1;i<length;i++){
    newArr.push(arr[i]);
  }
  return newArr;
};
...
const howMuchPencil = (str) => {
  ...
  //     let xx=new RegExp(`^.{0,${ss}`, "g") 
  //     let result = arr.replace(xx,"");
  //     ss--;
  //     console.log(ss)
  //     newArr.push(result)
  // }
